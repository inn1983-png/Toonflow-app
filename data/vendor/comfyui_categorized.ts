 ...
 * - 支持角色图、场景图、道具图、衍生资产图、通用分镜图、LTX2.3 四宫格视频。
...
exports.vendor = {
  ...
  inputs: [
    ...
    { key: "deriveWorkflow", label: "衍生资产图 workflow JSON", type: "text", required: false, placeholder: "衍生资产图 API workflow JSON（图生图）" },
  ...
  models: [
    ...
    { name: "ComfyUI 衍生资产图工作流", modelName: "comfyui-derive-image", type: "image", mode: ["singleImage"] },
...
function workflowKeyForModel(modelName) {
  ...
  if (modelName === "comfyui-derive-image") return "deriveWorkflow";
...
function buildImagePrompt(input, modelName) {
  ...
  if (modelName === "comfyui-derive-image") {
...
exports.imageRequest = async function imageRequest(input, model) {
  ...
  if (modelName === "comfyui-derive-image") {
    if (!references.length) throw new Error("衍生资产图需要至少 1 张参考图。");
    const imageName = await uploadImage(references[0].base64, "toonflow_derive_ref");
    patchImage(working, getInputValue("deriveImageNodeId", "78"), imageName);
    patchText(working, getInputValue("derivePromptNodeId", "102:76"), String(input.prompt || "").trim());
    patchText(working, getInputValue("deriveNegativeNodeId", "102:77"), getInputValue("defaultImageNegative", ""));
  } else {
